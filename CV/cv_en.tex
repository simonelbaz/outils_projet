% $Author: $ $Date: $
% $Header: $ $Name:  $
% $Revision: $
%

\documentclass[a4paper,11pt]{article}
\usepackage{latexsym,graphicx,multicol}
\usepackage[T1]{fontenc}
\usepackage[french]{babel}
\usepackage{framed,longtable,tabularx,ltxtable}
\usepackage{wrapfig,multirow}
\pagestyle{empty}
\usepackage{vmargin}            % redéfinir les marges
\setmarginsrb{1cm}{2cm}{2cm}{1cm}{0cm}{0cm}{0cm}{0cm}

\begin{document}
\textbf{Simon ELBAZ\hspace{\stretch{1}} 8, rue Belgrand\hspace{\stretch{1}} 92300 Levallois-Perret} \\

%\begin{minipage}{10cm}
%\begin{tabularx}{\textwidth}{lX}
\begin{tabular*}{7in}{l@{\extracolsep{\fill}}r}
Mob :    +33 6 14 99 02 78 & \multirow{3}{*}{\includegraphics[width=3cm,height=3cm]{photo.eps}} \\
E-mail : simon.elbaz@free.fr & \\
Born $7^\mathrm{th}$ July 1975, Casablanca, married, 2 children & \\
& \\
& \\
%\end{minipage}
%\end{tabularx}
\end{tabular*}
\bigskip
%\hfill

%\begin{wrapfigure}{R}{0.2\textwidth}
%\begin{figure}[!h]
%\centering
%\includegraphics[width=2cm,height=3cm]{photo.eps}
%\end{figure}
%\caption{test}
%\end{wrapfigure}

\center{\textbf{Engineer from Computer Sciences and Applied Mathematics department at E.N.S.E.E.I.H.T (www.enseeiht.fr)}}
\bigskip

\begin{tabularx}{\textwidth}{X}
\hline
\textbf{Professional Skills}\\
\hline
\begin{itemize}
\item Customer need analysis, good programmer, testing 
\item Good telco operator rating\slash billing systems knowledge  
\item Call record processing, Dunning, CRM 
\item Revenue Management, French driving License 
\item Internet Protocols (IPv4, IPv6), Web 2.0 
\end{itemize}
\end{tabularx}

\begin{tabularx}{\textwidth}{X}
\hline
\textbf{Technical Skills}\\
\hline
\begin{itemize}
\item    \textbf{Software}: BSCS, Kenan FX, Oracle BRM, Arbor BP, Zope3, Plone, Subversion, Valgrind, Insure++, Git, Talend, Datastage, Informatica, Telelogic Continuus
\item    \textbf{Database}: Oracle, Postgresql, MySQL, SQLite, Sybase
\item    \textbf{OS} : Linux, Unix, NT
\item    \textbf{Languages} : C/C++, Python, Cython, Shell, Perl, Java, PHP, Lex, Yacc, PL/SQL, Pro*C
\item    \textbf{Networks} : SSL, IPv6
\end{itemize}
\end{tabularx}

\begin{tabularx}{\textwidth}{X}
\hline
\textbf{References}\\
\hline
\begin{itemize}
\item \textbf{Capgemini} : Test Team Responsible / Programmer at Capgemini
\item \textbf{Tech Data} : Marketing training (6 weeks) at Tech Data Corporation
\end{itemize}
\end{tabularx}

\begin{tabularx}{\textwidth}{X}
\hline
\textbf{Languages}\\
\hline
\begin{itemize}
\item    \textbf{English} : fluent (TOEIC : 740/990)
\item    \textbf{French} : mother tongue
\end{itemize}
\end{tabularx}

\begin{tabularx}{\textwidth}{p{0.2\textwidth}X}
\hline
\textbf{Education} & \\
\hline

1996-99     & Student at School of Engineering ENSEEIHT in the Computer Sciences and Applied Mathematics department \\
1993-96     &  Preparatory Classes for Grandes Ecoles in Mathematics, Physics and Chemistry at Lyc\'ee Carnot, Paris \\
1993     &  Baccalaureat in Mathematics and Physics with honors at Lyc\'ee Lyautey1, Casablanca, Morocco \\
\end{tabularx}

\LTXtable{\textwidth}{cv_en_experience.tex}

\begin{tabularx}{\textwidth}{X}
\hline
\textbf{Hobbies :}\\
\hline
\begin{itemize}
\item Swimming, books
\end{itemize}
\end{tabularx}

\flushright{Levallois, $22^\mathrm{th}$ august 2012}

\end{document}
