...
. ~/.profile_appli
. ${HOME_APP_FAETON}/param/GenLoader.cfg

sqlplus ${FAETON_DB_LOGIN}/${FAETON_DB_PASSWORD}@${FAETON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from all_constraints where status <> 'ENABLED';
EOF

NB_KO=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`
...
NB_DECISION_ADM_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from DECISION_ADMINISTRATIVE;
EOF

NB_DECISION_ADM_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_DECISION_ADM_ABANDONNED=`cut -f35 -d'§' ${HOME_APP_FAETON}/tmp/mig/out_cit/SANC.* | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_decision_administrative_pref.txt  | wc -l`

NB_DECISION_ADM_IN=`cut -c 159-160 ${HOME_APP_FAETON}/in/FE9SANC-C002.dat | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_decision_administrative_pref.txt | wc -l`
...
NB_DOSSIER_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from DOSSIER;
EOF

NB_DOSSIER_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_DOSSIER_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/DOSC.* | tail -1 | sed -e 's/ total$//'`
NB_DOSSIER_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/DOSC.* | wc -l`

NB_DOSSIER_IN=`wc -l ${HOME_APP_FAETON}/in/FE9DOSC-C002.dat | sed -e 's/ .*$//'`
...
NB_TITRE_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from TITRE;
EOF

NB_TITRE_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_TITRE_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/TITR.* | tail -1 | sed -e 's/ total$//'`
NB_TITRE_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/TITR.* | wc -l`

NB_TITRE_IN=`wc -l ${HOME_APP_FAETON}/in/FE9TITR-C002.dat | sed -e 's/ .*$//'`
...
NB_CATEGORIE_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from CATEGORIE;
EOF

NB_CATEGORIE_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_CATEGORIE_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/DROI.* | tail -1 | sed -e 's/ total$//'`
NB_CATEGORIE_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/DROI.* | wc -l`

NB_CATEGORIE_IN=`wc -l ${HOME_APP_FAETON}/in/FE9DROI-C002.dat | sed -e 's/ .*$//'`
...
NB_TETR_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from TITRE_FR_ETRANGER;
EOF

NB_TETR_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_TETR_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/TETR.?-? | tail -1 | sed -e 's/ total$//'`
NB_TETR_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/TETR.?-? | wc -l`

NB_TETR_IN=`wc -l ${HOME_APP_FAETON}/in/FE9TETR-C002.dat | sed -e 's/ .*$//'`
...
NB_AVIM_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from AVIS_MEDICAL;
EOF

NB_AVIM_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_AVIM_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/AVIM.?-? | tail -1 | sed -e 's/ total$//'`
NB_AVIM_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/AVIM.?-? | wc -l`

NB_AVIM_IN=`wc -l ${HOME_APP_FAETON}/in/FE09SL002F27PCT1.dat | sed -e 's/ .*$//'`
...
# EOF
# 
# NB_SLR48_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`
# 
# sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
# select 'result=' || count(*) from AVIS_MEDICAL;
# EOF
# 
# NB_SLR48_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`
# 
# NB_SLR48_ABANDONNED=`wc -l ${HOME_APP_FAETON}/tmp/mig/out_cit/SLR48.?-? | tail -1 | sed -e 's/ total$//'`
# NB_SLR48_ABANDONNED_ENTETE=`ls -1 ${HOME_APP_FAETON}/tmp/mig/out_cit/SLR48.?-? | wc -l`
# 
# NB_SLR48_IN=`wc -l ${HOME_APP_FAETON}/in/FE09SL002F27PCT1.dat | sed -e 's/ .*$//'`
# 
# echo ${NB_SLR48_IN}
...
NB_AF_AFM_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from AF_AFM;
EOF

NB_AF_AFM_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_AF_AFM_ABANDONNED=`cut -f35 -d'§' ${HOME_APP_FAETON}/tmp/mig/out_cit/SANC.* | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_af_afm.txt  | wc -l`

NB_AF_AFM_IN=`cut -c 159-160 ${HOME_APP_FAETON}/in/FE9SANC-C002.dat | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_af_afm.txt | wc -l`
...
NB_DECISION_JUDICIAIRE_FAETON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

sqlplus ${TAMPON_DB_LOGIN}/${TAMPON_DB_PASSWORD}@${TAMPON_ORACLE_SID} <<EOF > ~/save_cfg/log/result.log
select 'result=' || count(*) from DECISION_JUDICIAIRE;
EOF

NB_DECISION_JUDICIAIRE_TAMPON=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`

NB_DECISION_JUDICIAIRE_ABANDONNED=`cut -f35 -d'§' ${HOME_APP_FAETON}/tmp/mig/out_cit/SANC.* | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_decision_judiciaire.txt  | wc -l`

NB_DECISION_JUDICIAIRE_IN=`cut -c 159-160 ${HOME_APP_FAETON}/in/FE9SANC-C002.dat | grep -f ${HOME_USER_FAETON}/save_cfg/liste_codes_sanc_infraction_decision_judiciaire.txt | wc -l`
...
NB_LIGNES_REMONTEES=`grep 'result=' ~/save_cfg/log/result.log | cut -f2 -d'='`
...
do
NB_LIGNES_IN=`wc -l ${HOME_USER_FAETON}/save_cfg/ref_solution/REFERENTIEL/${a_tab}".csv" | sed -e 's/ .*$//'`
